...
/* when libc is not available, you shall implement the below memory interfaces (libc equivalents). */
    ...
    #define nnom_malloc(n)      tos_mmheap_aligned_alloc(n, 4)
    #define nnom_free(p)        tos_mmheap_free(p)
#endif
...
/* runtime & debug */
#define nnom_us_get()       tos_systick_get()*1000       // return a microsecond timestamp // os的刻度为ms
#define nnom_ms_get()       tos_tick2millisec(tos_systick_get())       // return a millisecond timestamp
